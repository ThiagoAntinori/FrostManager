...
namespace DAL.Implementations.SqlServer
{
    public class VentaRepository : IVentaRepository
    {
        public void Delete(Venta obj, UnitOfWork uow = null)
        {
            try
            {
                SqlHelper.ExecuteNonQuery("UPDATE Venta SET Borrado = 1 WHERE IdVenta = @IdVenta",
                                            CommandType.Text,
                                            uow?.Transaction,
                                            new SqlParameter[]
                                            {
        ...
        public IEnumerable<Venta> GetAll()
        {
            try
            {
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader("SELECT IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta FROM Venta WHERE Borrado = 0",
                    CommandType.Text,
                    new SqlParameter[]
                    {}))
                {
        ...
        public IEnumerable<Venta> GetByPeriodo(DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader("SELECT IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta FROM Venta WHERE Borrado = 0 AND Fecha BETWEEN @FechaInicio AND @FechaFin",
                    CommandType.Text,
                    new SqlParameter[]
                    {
        ...
        public IEnumerable<Venta> GetByFecha(DateTime fecha)
        {
            try
            {
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader("SELECT IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta FROM Venta WHERE Fecha = @Fecha AND Borrado = 0",
                    CommandType.Text,
                    new SqlParameter[]
                    {
        ...
        public IEnumerable<Venta> GetByMedioPago(MedioPago medioPago)
        {
            try
            {
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader("SELECT IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta FROM Venta WHERE IdMedioPago = @IdMedioPago AND Borrado = 0",
                    CommandType.Text,
                    new SqlParameter[]
                    {
        ...
        public void Insert(Venta obj, UnitOfWork uow)
        {
            try
            {
                SqlHelper.ExecuteNonQuery("INSERT INTO Venta (IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta, Borrado) VALUES (@IdVenta, @EsDelivery, @Fecha, @Hora, @IdMedioPago, @IdEstadoVenta, 0)",
                    CommandType.Text,
                    uow?.Transaction,
                    new SqlParameter[]
                    {
        ...
        public IEnumerable<Venta> GetByEstado(EstadoVenta estadoVenta)
        {
            try
            {
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader("SELECT IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta FROM Venta WHERE IdEstadoVenta = @IdEstadoVenta AND Borrado = 0",
                    CommandType.Text,
                    new SqlParameter[]
                    {
        ...
        public Venta GetVentaEnCurso()
        {
            try
            {
                ...
                string selectQuery = @"SELECT TOP 1 IdVenta, EsDelivery, Fecha, Hora, IdMedioPago, IdEstadoVenta
                                        FROM Venta WHERE IdEstadoVenta = @IdEstadoVenta AND Borrado = 0  
                ...
                using (SqlDataReader reader = SqlHelper.ExecuteReader(selectQuery,
                    ...
                    new SqlParameter[]
                    {
                        new SqlParameter("@IdEstadoVenta", (int)EstadoVenta.EnCurso)
                    }))
                {
                    object[] values = new object[reader.FieldCount];
